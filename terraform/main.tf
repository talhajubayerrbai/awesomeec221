...
# Break the cycle: ALB->App ingress rule as standalone resource
...
# 
# S3 Bucket for SSM connection plugin
# 
resource "random_id" "ssm_bucket_suffix" {
  byte_length = 4
}

resource "aws_s3_bucket" "ssm" {
  bucket        = "${var.project_name}-ssm-${random_id.ssm_bucket_suffix.hex}"
  force_destroy = true
...
output "ssm_bucket_name" {
  description = "S3 bucket name for SSM connection plugin"
